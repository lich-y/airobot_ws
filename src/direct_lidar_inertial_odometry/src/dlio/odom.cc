...
void dlio::OdomNode::getParams() {
  ...
  // 2D TF 发布模式
  dlio::declare_param(this, "odom/publish_2d_tf", this->publish_2d_tf_, false);
  if (this->publish_2d_tf_) {
    RCLCPP_INFO(this->get_logger(), "✅ 2D TF 模式已启用：odom→base_link 将只保留 yaw，去掉 roll/pitch，z=0");
  }
}
...
void dlio::OdomNode::publishPose() {
  ...
  // 🔍 诊断：检查 imu_stamp 和 scan_header_stamp 的差异
  static rclcpp::Time last_diag_time(0, 0, RCL_ROS_TIME);
  rclcpp::Time now = this->now();
  if (last_diag_time.seconds() == 0 || (now - last_diag_time).seconds() > 5.0) {
    double imu_t = rclcpp::Time(this->imu_stamp).seconds();
    double scan_t = rclcpp::Time(this->scan_header_stamp).seconds();
    double diff = scan_t - imu_t;
    RCLCPP_INFO(this->get_logger(), 
      "🔍 DLIO时间戳诊断: imu_stamp=%.3f, scan_stamp=%.3f, 差值=%.3fs (正=scan新)",
      imu_t, scan_t, diff);
    if (std::abs(diff) > 0.5) {
      RCLCPP_WARN(this->get_logger(), 
        "⚠️ imu_stamp 和 scan_stamp 差异过大 (%.3fs)，可能是IMU回调执行延迟", diff);
    }
    last_diag_time = now;
  }
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  ...
  if (this->publish_2d_tf_) {
    // 2D 模式：只保留 x, y, yaw，去掉 roll/pitch，z=0
    transformStamped.transform.translation.x = this->state.p[0];
    transformStamped.transform.translation.y = this->state.p[1];
    transformStamped.transform.translation.z = 0.0;

    // 从四元数提取 yaw，创建只有 yaw 的四元数
    double qw = this->state.q.w();
    double qx = this->state.q.x();
    double qy = this->state.q.y();
    double qz = this->state.q.z();
...
void dlio::OdomNode::getScanFromROS(const sensor_msgs::msg::PointCloud2::SharedPtr& pc) {
  ...
  // [DEBUG] 原始点云数量（从 ROS 消息转换后）
  size_t points_from_ros = original_scan_->points.size();
  ...
  // [DEBUG] 移除 NaN 后的点云数量
  size_t points_after_nan = original_scan_->points.size();

  // Crop Box Filter
  this->crop.setInputCloud(original_scan_);
  this->crop.filter(*original_scan_);
  
  // [DEBUG] Crop Box 过滤后的点云数量
  size_t points_after_crop = original_scan_->points.size();
  
  // [DEBUG] 如果点云数量异常，打印警告
  if (points_after_crop < 1000) {
    RCLCPP_WARN(this->get_logger(), 
      "[DIAG] Point cloud pipeline: ROS=%zu -> NaN_removed=%zu -> Crop=%zu (crop_size=%.2f)",
      points_from_ros, points_after_nan, points_after_crop, this->crop_size_);
  }
...
void dlio::OdomNode::preprocessPoints() {
  ...
  // Voxel Grid Filter
  size_t points_before_voxel = this->deskewed_scan->points.size();
  if (this->vf_use_) {
  ...
  // [DEBUG] 如果点云数量异常，打印完整处理链
  size_t points_final = this->current_scan->points.size();
  if (points_final < 1000) {
    RCLCPP_WARN(this->get_logger(), 
      "[DIAG] Preprocessing: deskewed=%zu -> voxel=%zu (vf_res=%.2f) | original=%zu",
      points_before_voxel, points_final, this->vf_res_, this->original_scan->points.size());
  }
